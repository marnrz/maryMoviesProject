...
export default function SingleMoviePages() {
  const { id } = useParams();
  ...
  // .filter((member) => member.job === "Writing")
  const listItem = [
    {
      icon: (
    ...
    //   ),
    //   labItem: "Coutries:",
    //   renderItem: `${production_countries.name}`,
    // },
  ];
  ...
  function renderAuthor() {
    if (!Array.isArray(authorData)) return null;
    return authorData.map(({ known_for_department, name, id }, index) => {
      if (known_for_department === "Writing") {
        return (
          <p key={id} className="box ">
            {name}
  ...
  //   return directorData.map(({ known_for_department, name, id }) => {});
  // }
  return (
    <Style>
      <div
